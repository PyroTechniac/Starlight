...
export default class extends Language {
    public constructor(store: LanguageStore, file: string[], directory: string) {
        ...
        this.language = {
            DEFAULT: (key): string => `${key} wurde noch nicht für de-DE übersetzt.`,
            ...
            SETTING_GATEWAY_VALUE_FOR_KEY_NOEXT: (data, key): string => `Für das Attribut ${key} ist der Wert ${data} nicht vorhanden.`,
            SETTING_GATEWAY_VALUE_FOR_KEY_ALREXT: (data, key): string => `Für das Attribut ${key} ist der Wert ${data} bereits vorhanden.`,
            SETTING_GATEWAY_SPECIFY_VALUE: 'Du musst einen Wert angeben, um etwas hinzu zu fügen oder zu filtern.',
            SETTING_GATEWAY_KEY_NOT_ARRAY: (key): string => `Das Attribut ${key} ist kein Array.`,
            SETTING_GATEWAY_KEY_NOEXT: (key): string => `Das Attribut ${key} existiert nicht im aktuellen Datenschema.`,
            SETTING_GATEWAY_INVALID_TYPE: 'Der Parameter "type" muss entweder hinzugefügt oder gelöscht werden.',
            RESOLVER_INVALID_PIECE: (name, piece): string => `${name} muss ein gültiger ${piece} Name sein.`,
            RESOLVER_INVALID_MESSAGE: (name): string => `${name} muss eine gültige Message-ID sein.`,
            RESOLVER_INVALID_USER: (name): string => `${name} muss ein "mention" oder eine gültige User-ID sein.`,
            RESOLVER_INVALID_MEMBER: (name): string => `${name} muss eine Benutzererwähnung oder eine gültige User ID sein.`,
            RESOLVER_INVALID_CHANNEL: (name): string => `${name} muss eine Kanalerwähnung oder eine gültige Kanal ID sein.`,
            RESOLVER_INVALID_GUILD: (name): string => `${name} muss eine gültige Server-ID sein.`,
            RESOLVER_INVALID_ROLE: (name): string => `${name} muss eine Rollenerwähnung oder eine gültige Rollen ID sein.`,
            RESOLVER_INVALID_LITERAL: (name): string => `Deine Auswahl entspricht nicht der einzigen Möglichkeit: ${name}`,
            RESOLVER_INVALID_BOOL: (name): string => `${name} muss 'true' oder 'false' sein.`,
            RESOLVER_INVALID_INT: (name): string => `${name} muss eine Ganzzahl sein.`,
            RESOLVER_INVALID_FLOAT: (name): string => `${name} muss eine gültige Zahl sein.`,
            RESOLVER_INVALID_URL: (name): string => `${name} muss eine gültige URL sein.`,
            RESOLVER_STRING_SUFFIX: ' Buchstaben',
            RESOLVER_MINMAX_EXACTLY: (name, min, suffix): string => `${name} muss genau ${min}${suffix} sein.`,
            RESOLVER_MINMAX_BOTH: (name, min, max, suffix): string => `${name} muss zwischen ${min} und ${max}${suffix} sein.`,
            RESOLVER_MINMAX_MIN: (name, min, suffix): string => `${name} muss größer als ${min}${suffix} sein.`,
            RESOLVER_MINMAX_MAX: (name, max, suffix): string => `${name} muss kleiner als ${max}${suffix} sein.`,
            COMMANDMESSAGE_MISSING: 'Es fehlen ein oder mehrere Argumente am Ende der Eingabe.',
            COMMANDMESSAGE_MISSING_REQUIRED: (name): string => `${name} ist ein erforderliches Argument.`,
            COMMANDMESSAGE_MISSING_OPTIONALS: (possibles): string => `Fehlende notwendige Auswahl: (${possibles})`,
            COMMANDMESSAGE_NOMATCH: (possibles): string => `Deine Auswahl trifft auf keine der folgenden Möglichkeiten zu: (${possibles})`,
            MONITOR_COMMAND_HANDLER_REPROMPT: (tag, error, time): string => `${tag} | **${error}** | Sie haben **${time}** Sekunden, um einen gültigen Wert einzugeben. Mit "ABORT" brechen Sie den Vorgang ab.`,
            MONITOR_COMMAND_HANDLER_ABORTED: 'Abgebrochen',
            INHIBITOR_COOLDOWN: (remaining): string => `Sie haben gerade erst diesen Befehl benutzt. Der Befehl kann von Ihnen wieder in ${remaining} Sekunden verwendet werden.`,
            INHIBITOR_DISABLED: 'Dieser Befehl ist aktuell deaktiviert.',
            INHIBITOR_MISSING_BOT_PERMS: (missing): string => `Ungenügende Berechtigungen, es fehlt: **${missing}**`,
            INHIBITOR_PERMISSIONS: 'Ihnen fehlt die Berechtigung diesen Befehl auszuführen.',
            INHIBITOR_REQUIRED_SETTINGS: (settings): string => `Aufgrund der fehlenden Einstellung **${settings.join(', ')}** kann dieser Befehl nicht ausgeführt werden.`,
            INHIBITOR_RUNIN: (types): string => `Dieser Befehl kann nur in ${types} Kanälen ausgeführt werden`,
            INHIBITOR_RUNIN_NONE: (name): string => `Der ${name} Befehl wurde nicht für die Benutzung in mindestens einem Kanal aktiviert.`,
            COMMAND_UNLOAD: (type, name): string => `✅ Deaktiviert ${type}: ${name}`,
            COMMAND_TRANSFER_ERROR: '❌ Diese Datei wurde bereits übertragen oder hat nie existiert.',
            COMMAND_TRANSFER_SUCCESS: (type, name): string => `✅ Erfolgreich übertragen ${type}: ${name}`,
            COMMAND_TRANSFER_FAILED: (type, name): string => `Übertrag von ${type}: ${name} zum Client fehlgeschlagen. Bitte überprüfe deine Konsole.`,
            COMMAND_RELOAD: (type, name): string => `✅ Neu geladen ${type}: ${name}`,
            COMMAND_RELOAD_ALL: (type): string => `✅ Alle ${type} neu geladen.`,
            COMMAND_REBOOT: 'Am neustarten...',
            COMMAND_PING: 'Ping?',
            COMMAND_PINGPONG: (diff, ping): string => `Pong! (Hin und zurück dauerte: ${diff}ms. Herzschlag: ${ping}ms.)`,
            COMMAND_INVITE_SELFBOT: 'Wieso solltest du einen Einladungslink für einen Selfbot benötigen...',
            COMMAND_INVITE: (client): string[] => [
            ...
            COMMAND_ENABLE: (type, name): string => `+ Erfolgreich ${type}: ${name} aktiviert`,
            COMMAND_DISABLE: (type, name): string => `+ Erfolgreich ${type}: ${name} deaktiviert`,
            ...
            COMMAND_CONF_ADDED: (value, key): string => `Erfolgreich den Wert \`${value}\` zum Attribut: **${key}** hinzugefügt`,
            COMMAND_CONF_UPDATED: (key, response): string => `Erfolgreiche Aktualisierung des Attributs **${key}**: \`${response}\``,
            COMMAND_CONF_KEY_NOT_ARRAY: 'Dieses Attribut ist nicht vom Typ "array". Bitte benutzen Sie stattdessen die Aktion \'reset\'.',
            COMMAND_CONF_REMOVE: (value, key): string => `Erfolgreich den Wert \`${value}\` vom Attribut: **${key}** entfernt`,
            COMMAND_CONF_GET_NOEXT: (key): string => `Es scheint so, dass das Attribut **${key}** nicht existiert.`,
            COMMAND_CONF_GET: (key, value): string => `Der hinterlegte Wert des Attributs **${key}** ist: \`${value}\``,
            COMMAND_CONF_RESET: (key, response): string => `Das Attribut **${key}** wurde zurückgesesetzt auf: \`${response}\``
        };
    }
