...
export class ClientUtil {
    public constructor(public client: StarlightClient) { }
    ...
    public resolvePermissionNumber(n: number): string[] {
        const resolved: string[] = [];
        for (const key of Object.keys(Discord.Permissions.FLAGS)) {
            if (n & Discord.Permissions.FLAGS[key]) resolved.push(key);
        }
    ...
    public attachment(file: string | Buffer | Stream, name?: string, data?: object): Discord.MessageAttachment {
        return new Discord.MessageAttachment(file, name, data);
    }

    public list<V>(iterable?: readonly V[]): List<V> {
        return new List<V>(iterable);
    }
