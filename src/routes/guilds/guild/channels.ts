...
import { FlattenedGuildChannel, Transformer } from '../../../lib/util/ApiTransform';

@SetRoute('guilds/:guild/channels')
export default class extends Route {
	...
	public async get(request: ApiRequest, response: ApiResponse): Promise<void> {
		...
		return response.json(guild.channels.map((chan): FlattenedGuildChannel => Transformer.resolve(chan).transform()));
	}
