	...
	public async run(arg: string, possible: Possible, message: KlasaMessage, filter?: (entry: string) => boolean): Promise<User> {
		...
		if (result) {
			return this.client.users.fetch(result[0])
				.catch((): never => { throw message.language.get('USER_NOT_EXISTENT'); });
		}

		throw message.language.get('RESOLVER_NO_RESULTS', possible.name, 'user');
	}

	public resolveUser(message: KlasaMessage, query: string): Promise<User | null> {
