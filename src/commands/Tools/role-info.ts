...
export default class RoleInfoCommand extends Command {
    ...
    public run(msg: KlasaMessage, [role = msg.member!.roles.highest]): Promise<KlasaMessage | KlasaMessage[]> {
        const allPermissions = Object.entries(role.permissions.serialize()).filter((perm): boolean => perm[1]).map(([perm]): string => this.perms[perm]).join(', ');
