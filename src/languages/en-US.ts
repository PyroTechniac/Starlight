    ...
    public constructor(store: LanguageStore, file: string[], directory: string) {
        ...
        this.language = {
            ...
            PREFIX_REMINDER: (prefix = `@${this.client.user!.tag}`): string => `The prefix${Array.isArray(prefix) ?
                `es for this guild are: ${prefix.map((pre): string => `\`${pre}\``).join(', ')}` :
            ...
            SETTING_GATEWAY_CHOOSE_KEY: (keys): string => `Please, choose one of the following keys: '${keys}'`,
            ...
            RESOLVER_INVALID_STORE: (store): string => `${store} must be a valid store name that is located in client.pieceStores`,
            ...
            RESOLVER_MINMAX_EXACTLY: (name, min): string => `${name} must be exactly ${min}.`,
            RESOLVER_MINMAX_BOTH: (name, min, max, inclusive): string => inclusive ? `${name} must be between ${min} and ${max} inclusively.` : `${name} must be between ${min} and ${max} exclusively.`,
            RESOLVER_MINMAX_MIN: (name, min, inclusive): string => inclusive ? `${name} must be greater than ${min} inclusively.` : `${name} must be greater than ${min} exclusively.`,
            RESOLVER_MINMAX_MAX: (name, max, inclusive): string => inclusive ? `${name} must be less than ${max} inclusively` : `${name} must be less than ${max} exclusively.`,
            ...
            INHIBITOR_COOLDOWN: (remaining): string => `You have just used this command. You can use this command again in ${remaining} second${remaining === 1 ? '' : 's'}.`,
