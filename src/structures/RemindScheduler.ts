...
export default class RemindScheduler {
	...
	protected queuedSchedules: Map<string, NodeJS.Timeout> = new Map();

	public constructor(client: StarlightClient, repository: Repository<any>, { checkRate = 5 * 60 * 1000 } = {}) {
		this.client = client;
	...
	public async runReminder(reminder: Reminder): Promise<void> {
		try {
			const reason = reminder.reason || `${reminder.channel ? 'y' : 'Y'}ou wanted me to remind you`
			...
			this.client.console.error(`[REMINDER ERROR] ${error.message}`, error.stack);
		}
