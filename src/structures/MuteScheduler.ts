...
export default class MuteScheduler {
    ...
    protected queuedSchedules: Map<number, NodeJS.Timeout> = new Map()
    public constructor(protected client: StarlightClient, protected repo: Repository<any>, { checkRate = 5 * 60 * 1000 } = {}) {
    ...
    public async cancelMute(mute: Case): Promise<boolean> {
        this.client.console.info(`[MUTE] Unmuted ${mute.target_tag} on ${this.client.guilds.get(mute.guild)}`);
    ...
    public rescheduleMute(mute: Case): void {
        this.client.console.info('Rescheduling mute');
        const schedule = this.queuedSchedules.get(mute.id);
        if (schedule) this.client.clearTimeout(schedule);
        this.queuedSchedules.delete(mute.id);
    ...
    public async addMute(mute: Case, reschedule: boolean = false): Promise<void> {
        this.client.console.info(`[MUTE] Muted ${mute.target_tag} on ${this.client.guilds.get(mute.guild)}`);
        if (reschedule) this.client.console.info(`[MUTE] Rescheduled mute on ${mute.target_id} on ${this.client.guilds.get(mute.guild)}`);
        if (!reschedule) {
            const casesRepo = this.client.db.getRepository(Case);
            const cs = new Case();
            cs.guild = mute.guild;
