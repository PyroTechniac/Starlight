...
export default class extends Provider {
    ...
    public deleteTable(table: string): Promise<void> {
        return this.hasTable(table)
            .then((exists) => exists ? fs.emptyDir(resolve(this.baseDirectory, table)).then(() => fs.remove(resolve(this.baseDirectory, table))) : undefined);
    }
    ...
    public get(table: string, id: string): Promise<any> {
        return tomlUtils.readTOML(resolve(this.baseDirectory, table, `${id}.toml`)).catch(() => null);
    }
