...
export default class extends Provider {
	...
	public async getKeys(table: string, entries: string[] = []): Promise<any[]> {
		if (entries.length) {
			const chunks = util.chunk(entries, 50000);
			const output = [];
			// @ts-ignore
	...
	public create(table: string, id: string, value: object = {}): Promise<WriteResult<any>> {
		return this.db.table(table).insert({ ...this.parseUpdateInput(value), id }).run();
	}

	public update(table: string, id: string, value: object = {}): Promise<WriteResult<any>> {
		return this.db.table(table).get(id).update(this.parseUpdateInput(value))
			.run();
	}

	public replace(table: string, id: string, value: object = {}): Promise<WriteResult<any>> {
		return this.db.table(table).get(id).replace({ ...this.parseUpdateInput(value), id })
			.run();
	}

	public delete(table: string, id: string): Promise<WriteResult<any>> {
